...
test_case = {
  'name' : 'Test Client - Many Connections',
  'description' : 'Many clients can connect to the server and a message passes from one client to all other clients.',
  'points_possible' : 20,
  'points_earned' : 0,
  'test_ran' : False,
  'test_passed' : False,
  ...
  if ( server.poll() != None or ( not all_clients_running ) ):
    ...
    with open( server_out.name ) as file:
      for line in file:
        print( line.rstrip() )
    print('-' * 10 )
    for index, out in zip(range(6), client_out):
    ...
    print('Sending "hello world" from client 2 to server...')
    client_list[2].stdin.write('hello world\n')
    client_list[2].stdin.flush()
    time.sleep(3)
    
    found_output_everywhere = True
    rx = re.compile('hello world', re.I)
    print('Checking all other clients for "hello world"...')
    print('')
    allClientsReceived = True
    for index, client, out in zip(range(6), client_list, client_out):
      if ( index != 2 ):
        client_output = ""
        ...
        if ( result ):
          print('Message arrived at client ' + str(index) )
        else:
          print('Message did not arrive at client ' + str(index) )
          allClientsReceived = False
        print('')
    #
    if ( allClientsReceived ):
      print('Message arrived at all other clients')
    else:
      print('Message failed to arrive at one or more clients')
      test_case['feedback'] += 'Message failed to arrive at one or more clients'
      test_failed = True
