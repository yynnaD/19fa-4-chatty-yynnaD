...
test_case = {
  'name' : 'Test DM - Multiple Match',
  'description' : 'The /dm command works correctly when multiple users have the nickname.',
  'points_possible' : 10,
  'points_earned' : 0,
  'test_ran' : False,
  'test_passed' : False,
...
def do_test( test_case, skip_remaining_tests ):
  ...
  if ( server.poll() != None or ( not all_clients_running ) ):
    ...
    with open( server_out.name ) as file:
      for line in file:
        print( line.rstrip() )
    print('-' * 10 )
    for index, out in zip(range(5), client_out):
    ...
    print('Sending "/nick" command from client 1 to server...')
    client_list[1].stdin.write('/nick rbessick5\n')
    client_list[1].stdin.flush()
    time.sleep(3)
    
    print('Sending "/nick" command from client 3 to server...')
    client_list[3].stdin.write('/nick rbessick5\n')
    client_list[3].stdin.flush()
    time.sleep(3)
    ...
    print('Checking output to ensure only clients 1 and 3 received the /dm...')
    rx = re.compile('super secret', re.I)
    
    nickClients = [1, 3]
    
    for index, client, out in zip(range(5), client_list, client_out):
      if ( index > 0 ):
        client_output = ""
  ...
  if ( test_failed ):
    print('The dm to one client did not work correctly')
    test_case['feedback'] = 'The dm to one client did not work correctly.'
  else:
    print('The dm to one client worked correctly')
